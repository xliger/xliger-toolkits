...
const blacklist: string[] = [
  ...
  "0x888888e8c80e2aeaeccf2e46be905c20e125a1db", // XLIGER
...
/**
 * Fetch Top100 Tokens traded on LigerSwap v2, ordered by trading volume,
...
const getTokenLogo = (address: string): string => {
  ...
  if (logoFiles.includes(`${address}.png`)) {
    return `https://tokens.ligerswap.org/images/${address}.png`;
  }
...
const main = async (): Promise<void> => {
  try {
    ...
    const tokenListPath = `${path.resolve()}/src/tokens/ligerswap-top-100.json`;
