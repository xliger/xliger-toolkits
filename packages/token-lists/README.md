# 🐯 LigerSwap Token Lists

This repo contains main LigerSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to LigerSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to LigerSwap (extended) token list
...
- Add new tokens to `src/tokens/ligerswap-extended.json` file
...
## How to update Top10 Token list

Note - this is not something we expect pull requests for.

```shell script
# Fetch the Top10 Tokens on LigerSwap v2, and update list.
$ yarn fetch:pcs-top-10

# Build token list (ligerswap-top-10.json)
$ yarn makelist:pcs-top-10
```
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.ligerswap.org/list-name.json`
